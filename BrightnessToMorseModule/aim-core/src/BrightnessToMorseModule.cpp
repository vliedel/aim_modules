  ...
  cliParam(0)
{
  ...
  portBrightnessIn.sock = new zmq::socket_t(*context, ZMQ_REP);
  portBrightnessIn.ready = true;
  zmq_sockets.push_back(&portBrightnessIn);
...
void BrightnessToMorseModule::Init(std::string & name) {
  ...
  
  {
    ...
    zmqPortName = "/brightnesstomorsemodule" + cliParam->module_id + "/brightness";
    portBrightnessIn.name = zmqPortName;
    zmqPortName = "/resolve" + portBrightnessIn.name;
    pns_record record;
    ...
    std::cout << "Bind to socket " << zmqPortName << std::endl;
    portBrightnessIn.sock->bind(zmqPortName.c_str());
  }
  {
    // outgoing port, function as server
    zmqPortName = "/brightnesstomorsemodule" + cliParam->module_id + "/morse";
...
void BrightnessToMorseModule::readCommands() {
  while (true) {
    HandleCommand();
  }
}
...
void BrightnessToMorseModule::Resolve(pns_record & record) {
...
void BrightnessToMorseModule::SendAck(zmq::socket_t *s, bool state) {
  if (debug) std::cout << "Send ACK" << std::endl;
  SendRequest(s, state, true, "ACK");
}

bool BrightnessToMorseModule::ReceiveAck(zmq::socket_t *s, bool & state, bool blocking) {
  int reply_size = 0;
...
char* BrightnessToMorseModule::GetReply(zmq::socket_t *s, bool & state, bool blocking, int & reply_size) {
  if (s == NULL)
    return NULL;
  zmq::message_t reply;
  char* result = NULL;
...
void BrightnessToMorseModule::SendRequest(zmq::socket_t *s, bool & state, bool blocking, std::string str) {
  if (state) {
...
void BrightnessToMorseModule::HandleCommand() {
  int reply_size = -1;
  bool state = false;
...
void BrightnessToMorseModule::Connect(std::string source, std::string target) {
...
zmq::socket_t* BrightnessToMorseModule::GetSocket(std::string name) {
...
float* BrightnessToMorseModule::readBrightness(bool blocking) {
  pthread_mutex_lock(&cmdMutex);
  int reply_size = -1;
  char *reply = GetReply(portBrightnessIn.sock, portBrightnessIn.ready, blocking, reply_size);
  if (!portBrightnessIn.ready) {
    delete [] reply;
    pthread_mutex_unlock(&cmdMutex);
    return NULL;
  }
  ...
  SendAck(portBrightnessIn.sock, portBrightnessIn.ready);
  pthread_mutex_unlock(&cmdMutex);
  if (reply_size < 1) std::cerr << "Error: Reply is not large enough to store a value!" << std::endl;
  std::stringstream ss; ss.clear(); ss.str("");
  ss << std::string(reply);
  ss >> portBrightnessValue;
  delete [] reply;
  return &portBrightnessValue;
}
