...
#ifndef FACERECOGMODULE_H_
#define FACERECOGMODULE_H_
...
private:
  ...
  long_seq portFaceImageValue;
  zmq_socket_ext portFaceImageIn;
  
  zmq_socket_ext portClassifiedOut;
  
  std::string portCmdValue;
  zmq_socket_ext portCmdIn;
  
  static void* readCommandsHelper(void* object) {
    ((FaceRecogModule*)object)->readCommands();
    return NULL;
  }
...
public:
  ...
  // Default destructor
  virtual ~FaceRecogModule();
  
  // Extend this with your own code, first call FaceRecogModule::Init(name);
  ...
  /**
   * The "readFaceImage" function receives stuff over a zeromq REP socket. It works as a client. It is better not
  ...
  long_seq *readFaceImage(bool blocking=false);
  
  /**
   * The "writeClassified" function sends stuff over a zeromq REQ socket. It works as a server. It cannot be blocking because this
  ...
  bool writeClassified(const int label);
  
  /**
   * The "readCmd" function receives stuff over a zeromq REP socket. It works as a client. It is better not
   * to run it in blocking mode, because this would make it impossible to receive message on other ports (under which 
   * the /pid/control port). The function returns NULL if there is no new item available.
   */
  // Read from this function and assume it means something
  // Remark: check if result is not NULL
  std::string *readCmd(bool blocking=false);
  
};
} // End of namespace

#endif // FACERECOGMODULE_H_
