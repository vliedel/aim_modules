...
public class JpgToBmpModuleService extends Service {
  ...
  private static final String MODULE_NAME = "JpgToBmpModule";
  private int mId = -1;
  ...
  private List<String> mPortJpgInBuffer = new ArrayList<String>(0);
  ...
  class PortJpgMessengerHandler extends Handler {
    ...
    public void handleMessage(Message msg) {
      ...
      case AimProtocol.MSG_PORT_DATA:
        ...
        synchronized(mPortJpgInBuffer) {
          mPortJpgInBuffer.add(readVal);
        }
      default:
...
//		handleStartCommand(intent);
  ...
  private class AIMRun extends AsyncTask<Integer, Void, Boolean> {
    protected Boolean doInBackground(Integer... id) {
      ...
      JpgToBmpModuleExt aim = new JpgToBmpModuleExt();
      //aim.Init("0"); // TODO: pass on the id
      AndroidBmpRead_t outputBmp;
      
      while (true) {
        synchronized(mPortJpgInBuffer) {
          if (!mPortJpgInBuffer.isEmpty()) {
            aim.androidWriteJpg(mPortJpgInBuffer.get(0));
            mPortJpgInBuffer.remove(0);
          }
        }
        
        aim.Tick();
        outputBmp = aim.androidReadBmp();
        if (outputBmp.getSuccess()) {
          
          // Debug
          StringBuffer str = new StringBuffer();
          str.append("outputBmp=");
          str.append(outputBmp.getVal().toString());
          str.append(" ");
          int[] data = new int[(int) outputBmp.getVal().size()];
          for (int i=0; i<outputBmp.getVal().size(); i++) {
            data[i] = outputBmp.getVal().get(i);
            str.append(outputBmp.getVal().get(i));
            str.append(" ");
          }
  ...
  // static constructor
  static {
    System.loadLibrary("JpgToBmpModule");
  }
}
