...
ImageDisplayModule::ImageDisplayModule():
  ns_socket(NULL),
  cmd_socket(NULL),
  debug(0),
  cliParam(0)
...
void ImageDisplayModule::Init(std::string & name) {
...
void ImageDisplayModule::Resolve(pns_record & record) {
...
void ImageDisplayModule::SendAck(zmq::socket_t *s, bool state) {
  if (debug) std::cout << "Send ACK" << std::endl;
  SendRequest(s, state, true, "ACK");
}

bool ImageDisplayModule::ReceiveAck(zmq::socket_t *s, bool & state, bool blocking) {
  int reply_size = 0;
...
char* ImageDisplayModule::GetReply(zmq::socket_t *s, bool & state, bool blocking, int & reply_size) {
  if (s == NULL)
    return NULL;
  zmq::message_t reply;
  char* result = NULL;
...
void ImageDisplayModule::SendRequest(zmq::socket_t *s, bool & state, bool blocking, std::string str) {
  if (state) {
...
void ImageDisplayModule::HandleCommand() {
  int reply_size = -1;
  bool state = false;
...
void ImageDisplayModule::Connect(std::string source, std::string target) {
...
zmq::socket_t* ImageDisplayModule::GetSocket(std::string name) {
...
long_seq* ImageDisplayModule::readImage(bool blocking) {
  pthread_mutex_lock(&cmdMutex);
  int reply_size = -1;
