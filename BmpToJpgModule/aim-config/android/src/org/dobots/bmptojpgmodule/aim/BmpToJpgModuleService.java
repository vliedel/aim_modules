  ...
  class PortBmpMessengerHandler extends Handler {
    ...
    public void handleMessage(Message msg) {
      ...
      case AimProtocol.MSG_PORT_DATA:
        ...
        synchronized(mPortBmpInBuffer) {
          mPortBmpInBuffer.add(bufVal);
        }
      default:
...
//		handleStartCommand(intent);
  ...
  private class AIMRun extends AsyncTask<Integer, Void, Boolean> {
    protected Boolean doInBackground(Integer... id) {
      ...
      BmpToJpgModuleExt aim = new BmpToJpgModuleExt();
      //aim.Init("0"); // TODO: pass on the id
      AndroidJpgRead_t outputJpg;
      
      while (true) {
        synchronized(mPortBmpInBuffer) {
          if (!mPortBmpInBuffer.isEmpty()) {
            aim.androidWriteBmp(mPortBmpInBuffer.get(0));
            mPortBmpInBuffer.remove(0);
          }
        }
        
        aim.Tick();
        outputJpg = aim.androidReadJpg();
        if (outputJpg.getSuccess()) {
          // Debug
          Log.d(TAG, "outputJpg=" + outputJpg.getVal());
          Message msg = Message.obtain(null, AimProtocol.MSG_PORT_DATA);
          Bundle bundle = new Bundle();
          bundle.putInt("datatype", AimProtocol.DATATYPE_STRING);
          bundle.putString("data", outputJpg.getVal());
          msg.setData(bundle);
          msgSend(mPortJpgOutMessenger, msg);
        }
  ...
  // static constructor
  static {
    System.loadLibrary("BmpToJpgModule");
  }
}
