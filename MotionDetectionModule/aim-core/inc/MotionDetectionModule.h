...
#ifndef MOTIONDETECTIONMODULE_H_
#define MOTIONDETECTIONMODULE_H_
...
private:
  ...
  long_seq portImageValue;
  zmq_socket_ext portImageIn;
  
  zmq_socket_ext portMotionOut;
  
  static void* readCommandsHelper(void* object) {
    ((MotionDetectionModule*)object)->readCommands();
    return NULL;
  }
...
public:
  ...
  // Default destructor
  virtual ~MotionDetectionModule();
  
  // Extend this with your own code, first call MotionDetectionModule::Init(name);
  ...
  /**
   * The "readImage" function receives stuff over a zeromq REP socket. It works as a client. It is better not
  ...
  long_seq *readImage(bool blocking=false);
  
  /**
   * The "writeMotion" function sends stuff over a zeromq REQ socket. It works as a server. It cannot be blocking because this
...
#endif // MOTIONDETECTIONMODULE_H_
