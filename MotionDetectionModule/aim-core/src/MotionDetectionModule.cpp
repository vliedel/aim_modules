...
MotionDetectionModule::MotionDetectionModule():
  ...
  cliParam(0)
{
  const char* const channel[2] = {"readImage", "writeMotion"};
  cliParam = new Param();
  ...
  portMotionOut.sock = new zmq::socket_t(*context, ZMQ_REQ);
  portMotionOut.ready = true;
  zmq_sockets.push_back(&portMotionOut);
}

MotionDetectionModule::~MotionDetectionModule() {
  delete cliParam;
  delete portImageIn.sock;
  delete portMotionOut.sock;
}

void MotionDetectionModule::Init(std::string & name) {
  ...
  
  {
    ...
    zmqPortName = "/motiondetectionmodule" + cliParam->module_id + "/image";
  ...
  }
  {
    ...
    zmqPortName = "/motiondetectionmodule" + cliParam->module_id + "/motion";
    portMotionOut.name = zmqPortName;
    zmqPortName = "/resolve" + portMotionOut.name;
    pns_record record;
    record.name = zmqPortName;
    zmqSs.clear(); zmqSs.str("");
    zmqSs << getpid();
...
void MotionDetectionModule::readCommands() {
  while (true) {
    HandleCommand();
  }
}
...
void MotionDetectionModule::Resolve(pns_record & record) {
...
void MotionDetectionModule::SendAck(zmq::socket_t *s, bool state) {
  if (debug) std::cout << "Send ACK" << std::endl;
  SendRequest(s, state, true, "ACK");
}

bool MotionDetectionModule::ReceiveAck(zmq::socket_t *s, bool & state, bool blocking) {
  int reply_size = 0;
...
char* MotionDetectionModule::GetReply(zmq::socket_t *s, bool & state, bool blocking, int & reply_size) {
  if (s == NULL)
    return NULL;
  zmq::message_t reply;
  char* result = NULL;
...
void MotionDetectionModule::SendRequest(zmq::socket_t *s, bool & state, bool blocking, std::string str) {
  if (state) {
...
void MotionDetectionModule::HandleCommand() {
  int reply_size = -1;
  bool state = false;
...
void MotionDetectionModule::Connect(std::string source, std::string target) {
...
zmq::socket_t* MotionDetectionModule::GetSocket(std::string name) {
...
long_seq* MotionDetectionModule::readImage(bool blocking) {
  pthread_mutex_lock(&cmdMutex);
  int reply_size = -1;
...
bool MotionDetectionModule::writeMotion(const int probability) {
  ...
  pthread_mutex_lock(&cmdMutex);
  bool state = portMotionOut.ready;
  SendRequest(portMotionOut.sock, state, false, ss.str());
  if (state) portMotionOut.ready = false;
  if (!portMotionOut.ready) {
    bool ack_state = true;
    ReceiveAck(portMotionOut.sock, ack_state, true);
    if (ack_state) {
      portMotionOut.ready = true;
      pthread_mutex_unlock(&cmdMutex);
      return true;
